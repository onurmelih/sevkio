...
async function ensureSchema() {
  await pool.query(`
    ...
    ALTER TABLE orders ADD COLUMN IF NOT EXISTS shipped_by INTEGER REFERENCES users(id);
    ...
    -- Çoklu depo desteği: her firma birden fazla depo/şube yönetebilir.
    ...
    -- Ürün stoğu artık depo bazında tutulur. products.stock, tüm depoların toplamının
    -- önbelleğe alınmış (cache) hâlidir — hızlı okuma ve pazaryeri senkronu için.
...
// Her firmanın en az bir "varsayılan" deposu olmasını garantiler (migration-safe).
// Zaten depo tanımlı firmalara dokunmaz. Mevcut ürün stoklarını da o depoya taşır.
async function ensureDefaultWarehouses() {
  const { rows: companies } = await pool.query(`SELECT id FROM companies`);
  for (const company of companies) {
    const { rows: existing } = await pool.query(
      `SELECT id FROM warehouses WHERE company_id=$1 LIMIT 1`, [company.id]
    );
    if (existing[0]) continue;

    const { rows: [warehouse] } = await pool.query(
      `INSERT INTO warehouses (company_id, name, is_default) VALUES ($1, 'Ana Depo', true) RETURNING id`,
      [company.id]
    );
    const { rows: products } = await pool.query(
      `SELECT id, stock FROM products WHERE company_id=$1`, [company.id]
    );
    for (const p of products) {
      await pool.query(
        `INSERT INTO warehouse_stock (warehouse_id, product_id, stock) VALUES ($1,$2,$3)
         ON CONFLICT (warehouse_id, product_id) DO NOTHING`,
        [warehouse.id, p.id, p.stock || 0]
      );
    }
  }
}
